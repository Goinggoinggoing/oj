...
class JudgeClient(object):
    def __init__(self, run_config, exe_path, max_cpu_time, max_memory, test_case_dir,
                 submission_dir, spj_version, spj_config, io_mode, img_files, output=False, other_out_file=None):
        ...
        self._other_out_file = other_out_file
        ...
        if self._img_files:
            ...
            for f in self._img_files:
                with open(f, 'rb') as im:
                    run_result['img'].append(base64.b64encode(im.read()).decode('utf-8'))

        if self._other_out_file:
            if self._other_out_file.endswith('.xlsx') or self._other_out_file.endswith('.xls'):
                file_reader = pandas.read_excel(self._other_out_file)
            elif self._other_out_file.endswith('.csv'):
                file_reader = pandas.read_csv(self._other_out_file, encoding='gbk')
            else:
               with open(self._other_out_file, 'r', encoding='UTF-8') as f:
                   file_reader = f.read()
            run_result['other_out_file_data'] = str(file_reader)

        run_result['img_files'] = self._img_files
